...
async function eatFood(foodName) {
  try {
    ...
    const food = bot.inventory.items().find(item => 
      foodName ? 
      item.name.includes(foodName.toLowerCase()) : 
    ...
    if (!food) {
      bot.chat(`I don't have any ${foodName || 'food'} to eat.`);
      return;
    }

    // Check if we need to eat
    if (bot.food === 20) {
      bot.chat("I'm not hungry right now.");
      return;
    }
    ...
    await bot.consume();
    bot.chat(`Ate ${food.name}. Food level: ${bot.food}`);

  } catch (err) {
    console.error('Eating error:', err);
...
// Add the handleDisconnect function
